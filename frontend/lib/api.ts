import type { AnalysisRequest, AnalysisResponse, Search, Tweet, Report, SavedSearch, User } from '@/types'
...
class ApiClient {
  ...
  async saveSearch(query: string, alertEnabled: boolean = false): Promise<SavedSearch> {
  ...
  // Reports
  async generateReport(searchId: string, format: 'pdf' | 'csv' | 'json'): Promise<Report> {
  ...
  async getCredits(): Promise<{ credits_remaining: number; credits_reset_date: string }> {
